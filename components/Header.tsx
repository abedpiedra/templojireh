      ...
      <div className="bg-dark text-white py-2 text-sm hidden md:block">
        <div className="container mx-auto px-4 flex justify-between items-center">
          <div className="flex gap-6">
            <span><i className="fas fa-phone text-primary mr-2"></i> +1 234 567 890</span>
            <span><i className="fas fa-envelope text-primary mr-2"></i> info@templojireh.com</span>
          </div>
        ...
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <nav className={`${isMenuOpen ? 'block' : 'hidden'} md:block absolute md:relative top-full left-0 right-0 bg-white md:bg-transparent shadow-md md:shadow-none`}>
            <ul className="flex flex-col md:flex-row md:gap-8 p-4 md:p-0">
              {navLinks.map((link) => (
                <li key={link.href} className="border-b md:border-none border-gray-100">
                  <Link
                    href={link.href}
