...
export default function EventoCard({ titulo, fecha, hora, lugar, descripcion, imagen }: EventoCardProps) {
  ...
  return (
    <div className="card flex flex-col md:flex-row">
      <div className="relative w-full md:w-72 h-48 md:h-auto flex-shrink-0">
        <Image
          src={imagen || 'https://images.unsplash.com/photo-1523580494863-6f3031224c94?w=600'}
          alt={titulo}
          fill
      ...
      <div className="p-6 flex-1 flex gap-6">
        ...
        <div className="flex-1">
          ...
          <div className="flex flex-wrap gap-4 text-sm text-gray-500 mb-4">
            <span><i className="fas fa-clock text-primary mr-1"></i> {hora}</span>
            <span><i className="fas fa-map-marker-alt text-primary mr-1"></i> {lugar}</span>
          </div>
          <a href="#" className="btn-primary text-sm py-2 px-4">Más Información</a>
        </div>
      </div>
    </div>
  )
